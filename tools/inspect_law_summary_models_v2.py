...
def _build_summary_input(data: dict) -> LawSummaryInput:
    ...
    revisions = [
        RevisionHistory(
            ...
            scheduled_enforcement_date=revision.get("scheduled_enforcement_date"),
...
def main() -> None:
    ...
    if not models:
        import config
        configured_model = existing_usage.get("model") or config.MODEL_NAME
        models = [f"{configured_model}@{config.LOCATION}"]
    ...
    original_location = gemini_client.LOCATION

    try:
        for model_spec in models:
            model_name, separator, location = model_spec.partition("@")
            if not separator or not model_name or not location:
                raise ValueError(
                    f"Invalid model specification: {model_spec}. "
                    "Use MODEL@LOCATION, e.g. gemini-3.5-flash@global."
                )
            ...
            gemini_client.LOCATION = location
        ...
        gemini_client.LOCATION = original_location
